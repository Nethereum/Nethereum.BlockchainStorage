...
namespace Nethereum.BlockchainStore.EF.Tests.Base.RepositoryTests
{
    public abstract class ContractRepositoryBaseTests: RepositoryTestBase
    {
        ...
        public async Task UpsertAsync()
        {
            ...
            Assert.IsFalse(await contractRepository.ExistsAsync(contractAddress));
            Assert.IsFalse(contractRepository.IsCached(contractAddress));

            await contractRepository.UpsertAsync(contractAddress, code, transaction);

            Assert.IsTrue(await contractRepository.ExistsAsync(contractAddress));
            Assert.IsTrue(contractRepository.IsCached(contractAddress));

            
            var storedContract = await context.Contracts.FindByContractAddressAsync(contractAddress);

            Assert.IsNotNull(storedContract);
            Assert.AreEqual(contractAddress, storedContract.Address);
            Assert.AreEqual(code, storedContract.Code);
            Assert.AreEqual(transaction.From, storedContract.Creator);
            Assert.AreEqual(transaction.TransactionHash, storedContract.TransactionHash);
